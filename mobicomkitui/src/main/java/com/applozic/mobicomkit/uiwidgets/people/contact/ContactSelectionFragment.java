...
public class ContactSelectionFragment extends ListFragment implements SearchListFragment,
                ...
                ApplozicChannelCreateTask.ChannelCreateListener channelCreateListener = new ApplozicChannelCreateTask.ChannelCreateListener() {
                    ...
                    public void onSuccess(Channel channel, Context context) {
                        ...
                        if (channel != null) {
                            ...
                            kirimKeteranganGroup(Integer.toString(channel.getKey()),sessionManager.getUserId(),userIdList);
                            if (ApplozicClient.getInstance(getActivity().getApplicationContext()).isContextBasedChat()) {
                                intent.putExtra(ConversationUIService.CONTEXT_BASED_CHAT, true);
                            }

                            //ambil dari sini id grup dan namanya
    ...
    public void kirimKeteranganGroup(String idGrup,String idAdmin,List<String> idAnggota) {
        Log.d("jadi","yey");
        try {
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("id_group", idGrup);
            jsonObject.put("id_admin",idAdmin);
            jsonObject.put("id_member",idAnggota);
            requestBody = jsonObject.toString();
            com.android.volley.RequestQueue requestQueue = Volley.newRequestQueue(getActivity().getApplicationContext());
            String url = AlamatServer.getAlamatServer()+ AlamatServer.getRegisterGroup();
            Log.d("jadi",url);
            StringRequest login = new StringRequest(Request.Method.POST, url
                ...
                public void onResponse(String response) {
                    try {
                        ...
                        Toast.makeText(getActivity(), jsonObject.get("status").toString(), Toast.LENGTH_SHORT).show();
