...
import com.applozic.mobicomkit.uiwidgets.meetup.CreateMeetupActivity;
...
import com.applozic.mobicomkit.uiwidgets.resumemeetup.MeetUpResume;
...
import com.applozic.mobicomkit.uiwidgets.vote.CreateVoteActivity;
import com.applozic.mobicomkit.uiwidgets.vote.Pilihan;
import com.applozic.mobicomkit.uiwidgets.vote.Vote;
...
abstract public class MobiComConversationFragment extends Fragment implements View.OnClickListener {
    ...
    private boolean meetUpKosong = true;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        sendButton.setOnClickListener(new View.OnClickListener() {
                                          @Override
                                          public void onClick(View view) {
                                              Toast.makeText(getActivity(), "Jan", Toast.LENGTH_SHORT).show();
                                              emoticonsFrameLayout.setVisibility(View.GONE);
...
//    }
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        if (channel != null) {
            ...
            menu.findItem(R.id.dial).setVisible(false);
            menu.findItem(R.id.video_call).setVisible(false);
            menu.findItem(R.id.vote).setVisible(true);
            menu.findItem(R.id.meet_up_resume).setVisible(true);
        ...
        if(channel!=null && channel.isDeleted()){
            ...
            menu.findItem(R.id.meetup).setVisible(true);
        }

        boolean isUserPresent = false;
        if (channel != null) {
            isUserPresent = ChannelService.getInstance(getActivity()).processIsUserPresentInChannel(channel.getKey());
        }
        if (isUserPresent || contact!=null ) {
            menu.findItem(R.id.meetup).setVisible(true);
        } else {
            menu.findItem(R.id.meetup).setVisible(false);
        }

//        if(!ChannelUtils.isAdminUserId(MobiComUserPreference.getInstance(getActivity().getApplicationContext()).getUserId(), channel)){
//            menu.findItem(R.id.meetup_resume).setVisible(false);
//        }else {
//            menu.findItem(R.id.meetup_resume).setVisible(true);
//        }
//
//        if(channel.isDeleted()){
////            if(!ChannelUtils.isAdminUserId(MobiComUserPreference.getInstance(getActivity().getApplicationContext()).getUserId(), channel)){
...
//        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if(id==R.id.meet_up_resume){
            if(channel != null) {
                Intent inte= new Intent(getActivity(), MeetUpResume.class);
                inte.putExtra(MeetUpResume.ID_USER,MobiComUserPreference.getInstance(getActivity().getApplicationContext()).getUserId());
                inte.putExtra(MeetUpResume.ID_GROUP,Integer.toString(channel.getKey()));
                inte.putExtra(MeetUpResume.OTHER_USER,"-");
                inte.putExtra(MeetUpResume.NAMA_GROUP,channel.getName());
                startActivity(inte);
            }else{
                Intent inte= new Intent(getActivity(), MeetUpResume.class);
                inte.putExtra(MeetUpResume.ID_USER,MobiComUserPreference.getInstance(getActivity().getApplicationContext()).getUserId());
                inte.putExtra(MeetUpResume.ID_GROUP,"-");
                inte.putExtra(MeetUpResume.OTHER_USER,contact.getUserId());
                inte.putExtra(MeetUpResume.NAMA_GROUP,"MeetUp Messenger");
                startActivity(inte);
            }
        }
        if(id==R.id.vote){
            if(ChannelUtils.isAdminUserId(MobiComUserPreference.getInstance(getActivity().getApplicationContext()).getUserId(), channel)) {
                ...
                intent.putExtra("id_group",Integer.toString(channel.getKey()));
                intent.putExtra("id_user",MobiComUserPreference.getInstance(getActivity().getApplicationContext()).getUserId());
        ...
        if(id==R.id.meetup){
            ...
            if(meetUpKosong){
                if(Utils.hasMarshmallow() && PermissionsUtils.checkSelfPermissionForLocation(getActivity())){
                    applozicPermission = new ApplozicPermissions(getActivity(), layout);
                    applozicPermission.requestLocationPermissions();
                }else {
                    if(channel==null){
                        cekMeetup(sm.getUserId(),contact.getUserId(),"-");
                    }else{
                        cekMeetup(sm.getUserId(),"-",Integer.toString(channel.getKey()));
                    }
                }
            }
        }
...
//    }
    ...
    public void cekMeetup(final String org1, final String org2, final String idGroup){
        Log.d("sayang",org1);
        Log.d("sayang",org2);
        Log.d("sayang",idGroup);
        final Meetup meetupO = new Meetup();
        RequestQueue requestQueue = Volley.newRequestQueue(getActivity().getApplicationContext());
        final String url = AlamatServer.getAlamatServer()+AlamatServer.getChekMeetup();
        Log.d("sayang",url);
        StringRequest login = new StringRequest(Request.Method.POST, url , new Response.Listener<String>(){
            ...
            public void onResponse(String response) {
                try {
                    ...
                    if(jsonObject.get("status").equals("ada")){
                        ...
                        JSONObject jsonObject1 = jsonObject.getJSONObject("meetup");
                        meetupO.setJudul(jsonObject1.getString("judul"));
                        meetupO.setAlamat(jsonObject1.getString("tempat"));
                        meetupO.setTanggal(jsonObject1.getString("tanggal"));
                        meetupO.setPukul(jsonObject1.getString("jam"));
                        meetupO.setId(jsonObject1.getString("id"));
                        meetupO.setLatitude(jsonObject1.getDouble("meetup_latitude"));
                        meetupO.setLongitude(jsonObject1.getDouble("meetup_longitude"));
                        if(channel==null){
                            DialogMeetUp dialogMeetUp = new DialogMeetUp(getActivity(),meetupO,org1,"true");
                            dialogMeetUp.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
                            dialogMeetUp.show();
                        }else{
                            if(ChannelUtils.isAdminUserId(MobiComUserPreference.getInstance(getActivity().getApplicationContext()).getUserId(), channel)) {
                                DialogMeetUp dialogMeetUp = new DialogMeetUp(getActivity(),meetupO,org1,"true");
                                ...
                                DialogMeetUp dialogMeetUp = new DialogMeetUp(getActivity(),meetupO,org1,"false");
                                dialogMeetUp.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
                                dialogMeetUp.show();
                            }
                        ...
                        if(channel==null){
                            DialogMeetUpKosong dialogMeetUpKosong = new DialogMeetUpKosong(getActivity(),org1,org2,"-",true);
                            dialogMeetUpKosong.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
                            dialogMeetUpKosong.show();
                        }else{
                            //cek apakah admin
                            if(ChannelUtils.isAdminUserId(MobiComUserPreference.getInstance(getActivity().getApplicationContext()).getUserId(), channel)) {
                                DialogMeetUpKosong dialogMeetUpKosong = new DialogMeetUpKosong(getActivity(),"-","-",Integer.toString(channel.getKey()),true);
                                dialogMeetUpKosong.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
                                dialogMeetUpKosong.show();

                            }else {
                                DialogMeetUpKosong dialogMeetUpKosong = new DialogMeetUpKosong(getActivity(),"-","-",Integer.toString(channel.getKey()),false);
                                dialogMeetUpKosong.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
                                dialogMeetUpKosong.show();
            ...
            protected Map<String, String> getParams() throws AuthFailureError {
                ...
                param.put("orang1",org1);
                param.put("orang2",org2);
                param.put("id_group",idGroup);
                return param;
            }
        };
    ...
    public void kirim(){
        if (contact != null && !contact.isBlocked() || channel != null) {
            ...
            if (TextUtils.isEmpty(messageEditText.getText().toString().trim()) && TextUtils.isEmpty(filePath)) {
                ...
                String inputMessage="Hai..Aku sudah buat jadwal meetup..cek ya";
                String[] inputMsg=inputMessage.toLowerCase().split(" ");
                List<String> userInputList = Arrays.asList(inputMsg);
                boolean disjointResult = (restrictedWords==null)? true : disjoint(restrictedWords, userInputList);
